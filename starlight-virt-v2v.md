...
## 5. Converting from VMware (vSphere / ESXi → KVM on Starlight)

VMware-to-KVM is the headline workflow for virt-v2v, so it gets its own
top-level section. Pick one of two paths depending on what you have
access to on the source side:

| You have... | Use | Section |
|-------------|-----|---------|
| Shell on a standalone or unmanaged ESXi host | `-i vmx -it ssh` | [5.1](#51-esxi-over-ssh--simple-vcenter-less) |
| vCenter (production, fleet) | `-ic vpx://... -it vddk` | [5.2](#52-vcenter--vddk--production-fast) |

Both paths end the same way: a qcow2 disk plus a libvirt domain XML
inside the distrobox, ready to `virsh define && virsh start` on
Starlight (see [§5.3](#53-defining-and-booting-the-converted-vm-on-starlight)).

### 5.1 ESXi over SSH — simple, vCenter-less
...
- Output `-o local -os /var/tmp` writes a libvirt domain XML plus
  ...
  as the live walk-through in [§6.4](#64-inspect-the-output).
...
- *Slow copy* — SSH is single-threaded and CPU-bound on the ESXi side.
  ...
  ([§5.2](#52-vcenter--vddk--production-fast)) which can read
  concurrently.

### 5.2 vCenter / VDDK — production, fast
...
### 5.3 Defining and booting the converted VM on Starlight

Both paths above leave you with `/var/tmp/<name>-sda` (qcow2) and
`/var/tmp/<name>.xml` (libvirt domain). To register and start on
Starlight's libvirtd:

```bash
distrobox enter fedora-virt -- bash -c '
  virsh -c qemu:///system define /var/tmp/<name>.xml
  virsh -c qemu:///system start  <name>
  virsh -c qemu:///system list   --all
'
```

Open the console with `virt-viewer -c qemu:///system <name>` (or
`virt-manager` if you exported the GUI app with `distrobox-export
--app virt-manager`).

---

## 6. Live Walk-through: Convert a Fedora 43 Cloud Image

The walk-through below uses a plain cloud-image qcow2 rather than a
real VMware source, because it (a) needs no external credentials,
(b) runs end-to-end in under a minute, and (c) exercises the exact
same conversion pass that runs on the VMware paths above.

### 6.1 Grab a source disk
...
### 6.2 Inspect with `virt-v2v-inspector` (dry-run)
...
### 6.3 Convert to a local libvirt directory
...
### 6.5 Boot the result on Starlight
...
Non-VMware sources, plus a few output variants. The VMware-specific
recipes live in [§5](#5-converting-from-vmware-vsphere--esxi--kvm-on-starlight).

### 7.1 Raw / qcow2 disk → local libvirt directory
...
### 7.2 OVA appliance → local libvirt
...
### 7.3 Convert directly into a libvirt pool
...
### 7.4 Just produce a bootable qemu command-line
...
### 7.5 In-place conversion (no copy)
...
## 8. Customisation at Conversion Time
...
## 10. Pre / Post-conversion Checklist
